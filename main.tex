...
We can extend \eqref{eq:systemHalfStrings} to have a spring force between $u_M$, $w_0$ and the average between them ($(u_M + w_0) / 2$)
\begin{align}
    &\begin{cases}\nonumber
        \delta_{tt}u^n &= c^2\delta_{xx}u_M^n - J(x_{u_M})F\\
        \delta_{tt}w^n &= c^2\delta_{xx}w_0^n + J(x_{u_M})F
    \end{cases}\\
    &\begin{cases}\nonumber
    ...
    \end{cases}\\
    &\begin{cases}\nonumber
        \delta_{tt}u_M^n &= c^2\delta_{xx}u_M^n + \beta \left(\frac{w_0^n - u_M^n}{2}\right)\\
        \delta_{tt}w_0^n &= c^2\delta_{xx}w_0^n - \beta\left( \frac{w_0^n - u_M^n}{2}\right)
    \end{cases}
    \\
    &\begin{cases}\label{eq:systemHalfStringsDispCorr}
        \delta_{tt}u_M^n &= c^2\delta_{xx}u_M^n + \beta \eta^n\\
        \delta_{tt}w_0^n &= c^2\delta_{xx}w_0^n - \beta\eta^n
    \end{cases}
\end{align}
with
...
\end{equation}
and $\beta = \beta(\alpha)$. We unfortunately can't just add the force like this as this might cause instability at the connection. Instead we need to add the centred temporal averaging operator $\mu_{t\cdot}$ which ensures stability \cite{Bilbao2009}.

\begin{equation}\label{eq:corrForceNoDamp}
    F = \beta \mu_{t\cdot}\eta^n.
\end{equation}
Setting $\beta$ inversely proportional to $\alpha$, i.e., the smaller $\alpha$ is the higher the `correction' effect, according to
\def\plusEps{+ \epsilon};
% \def\plusEps{};
...
\end{equation}
with $0<\epsilon \ll 1$ to prevent a division by 0. It will be shown that when calculating the force, a division by 0 can be prevented and $\epsilon = 0$ will still be defined. We can observe that when $\alpha = 0$ and the correction effect needs to be at a maximum, $\beta\rightarrow \infty$. When $\alpha \rightarrow 1$, $\beta \rightarrow 0$.
...
\end{align}
Using superscript $I$ to denote an intermediate state describing of $u^{n+1}$ and $w^{n+1}$ without the connection force, we also know, through Eq. \eqref{eq:etaCorrDef}, that 

\begin{align}
    \eta^{n+1} &= \frac{w_0^I-\frac{k^2}{h}F-\left(u_M^I+\frac{k^2}{h}F\right)}{2}\nonumber\\
    \eta^{n+1} &= \frac{w_0^I - u_M^I}{2} - \frac{k^2}{h}F,
\end{align}
...
\begin{equation}
    F = \left(\frac{w_0^I - u_M^I}{2}+r\eta^{n-1}\right)\left(\frac{h(1+\sigma_0/k)(1-\alpha)}{2h\alpha + k^2(1+\sigma_0/k)(1-\alpha)}\right).
\end{equation}
