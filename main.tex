...
An even better result is obtained using \textit{sinc interpolation}. One can retrieve a continuous-time signal $y(t)$ from a discrete time-series $y[n]$ using 
...
\end{equation}
where $\mathcal{D}_\text{i}$ is the range for interpolation. Ideally, one uses the entire domain of 
