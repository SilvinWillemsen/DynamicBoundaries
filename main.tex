...
Another, slightly trickier, setup could be that the left string has an extra point at its right side to cause some overlap. This might come in handy later if we want to interpolate the forces between the strings for better quality. See Figure \ref{fig:overlap}

\begin{figure}[h]
    \centering
    \subfloat[When $h$ changes.]{\label{fig:overlapFuul}{ \includegraphics[width=0.5\textwidth]{twoStringsOverlap.eps}}}
    \subfloat[Zoomed]{\label{fig:overlapZoom}{ \includegraphics[width=0.5\textwidth]{twoFreeStringsInterpolated.eps}}}
    \caption{}\label{eq:overlap}
\end{figure}
