...
\begin{equation}
    ...
    \end{cases}
\end{equation}
which, (again, recalling \eqref{eq:rigid}) are indeed equivalent expressions for the connected point.

Then, if we want to dynamically change the grid...

The previous is an exact solution to the problem when the stability condition is satisfied with equality..
...
\begin{figure}[h]
    ...
    \subfloat[At the left boundary.]{\label{fig:interpolatedZoom}{ \includegraphics[width=0.5\textwidth]{twoFreeStringsInterpolated.eps}}}
    \caption{Zoomed}
\end{figure}
\end{document}
