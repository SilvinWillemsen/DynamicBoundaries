...
\caption{\label{fig:addingPoint}{Point added to the grid according to \eqref{eq:addingRemovingPoints}}}   
\end{figure}
...
An even better result is obtained using \textit{sinc interpolation}. One can retrieve a continuous-time signal $y(t)$ from a descrete time-series $y[n]$ using 
\begin{equation}\label{eq:generalSinc}
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
Naturally, \eqref{eq:generalSinc} can not be implemented as is, due to the infinite sum. The continuous-time signal $y$ can, however, be quite well approximated even with a truncated version of Eq. \eqref{eq:generalSinc}.

Going from DSP back to FDTD methods, \eqref{eq:generalSinc} can be rewritten in terms we already know and applied to grid function $u$ in space (rather than time)
\begin{equation}\label{eq:generalSinc}
    u(x^\star) = \sum_{l = \mathcal{D}_\text{i}} u_l \cdot \text{sinc}\left(\frac{x^\star - lh}{h}\right).
\end{equation}
where $\mathcal{D}_\text{i}$ is the range for interpolation. Ideally, one uses the entire
...
\end{bmatrix}^T,
\end{equation}
(so $\mathbf{u}^n$ and $\mathbf{w}^n$ concatenated excluding the outer boundaries).

If $\alpha = 0$, $\mathbf{B}$ looks like
\begin{equation}
...
\begin{figure}[h]
    \centering
    \subfloat[Modal analysis. Modes are sorted at all times.]{\label{fig:modesAddInCenter}{ \includegraphics[width=0.45\textwidth]{modesAddPointsInCenter.eps}}}
    \hspace{0.05\textwidth}
    \subfloat[Spectrogram of system wave excited with raised cosine when $N = 15$.]{\label{fig:specAddCenter}{ \includegraphics[width=0.45\textwidth]{specAddCenter.eps}}}
